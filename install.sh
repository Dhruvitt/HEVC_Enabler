...
print_modname() {
  ui_print "*******************************"
  ui_print "   HEVC Enabler for Oneplus 3/3T/5/5T   "
  ui_print "*******************************"
}
...
if [ -f vendor/build.prop ]; then BUILDS="/system/build.prop vendor/build.prop"; else BUILDS="/system/build.prop"; fi
  ...
  OP5=$(grep -E "ro.product.device=oneplus5|ro.product.device=OnePlus5|ro.product.device=OnePlus5T" $BUILDS)
}
...
api_check() {
  if [ "$API" -eq 25 ] || [ "$API" -eq 24 ] || [ "$API" -eq 26 ] || [ "$API" -eq 27 ]; then
    break
  else
